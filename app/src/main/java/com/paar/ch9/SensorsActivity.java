...
public class SensorsActivity extends Activity implements SensorEventListener, LocationListener {
    ...
    private float[] prevGrav = new float[3];
        ...
        try {
            ...
            try {
                ...
                synchronized (magneticNorthCompensation) {
                    ...
                    magneticNorthCompensation.set((float) Math.cos(angleY), 0f, (float) Math.sin(angleY),
                                                  0f,                   1f, 0f,
    ...
    public void onSensorChanged(SensorEvent evt) {
        ...
        if (evt.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            //Log.d(TAG, "onSensorChanged: " + Arrays.toString(mag));
            mag[0] = smooth[0];
            mag[1] = smooth[1];
            mag[2] = smooth[2];
        }
        ...
        if (elapsedTime < 250){
            // если прошло меньше 250 миллисекунд - сохраним текущее время
            computing.set(false);
            return;
        }
...
//            Log.d(TAG, "onSensorChanged: " + (curGrav[0]) + "=" + prevGrav[0]);// + "=" + (curGrav[1]-prevGrav[1]) + "=" + (curGrav[2]-prevGrav[2]) );
//            prevGrav = curGrav;
